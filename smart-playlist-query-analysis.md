...
>
> 以上只是基础层。**每个关联模型自身可能还有 `$with`，会递归触发更多 SQL**——详见 §4.7 的递归展开分析。
...
|----------|---------|-----------------|--------------|
| `album` | Album | `['artist']`（[Album.php L63](file:///d:/fz/0601-1/solo-dogfeeding/code/19-koel/app/Models/Album.php#L63)） | 1 个 BelongsTo（album.artist） |
| `artist` | Artist | 无 | 0 |
| `album.artist` | Artist | 无 | 0 |
| `podcast` | Podcast | 无 | 0 |
| `genres` | Genre | 无 | 0 |
| `owner` | User | `['roles', 'permissions']`（[User.php L84](file:///d:/fz/0601-1/solo-dogfeeding/code/19-koel/app/Models/User.php#L84)） | 2 个 BelongsToMany（Spatie Permission） |
...
| # | 关联 | 关系类型 | 实际 SQL |
|---|------|---------|---------|
| 1 | `album` | BelongsTo | `SELECT * FROM albums WHERE id IN (...)` |
| 2 | `artist` | BelongsTo | `SELECT * FROM artists WHERE id IN (...)` |
| 3 | `album.artist` | BelongsTo (嵌套) | `SELECT * FROM artists WHERE id IN (...)` |
...
| # | 触发来源 | 关系类型 | 实际 SQL |
|---|---------|---------|---------|
| 8 | Album::$with → `artist` | BelongsTo | `SELECT * FROM artists WHERE id IN (...)` ← 与 #3 完全重复 |
| 9 | User::$with → `roles` (pivot) | BelongsToMany (Spatie) | `SELECT * FROM model_has_roles WHERE model_type = 'App\Models\User' AND model_id IN (...)` |
| 10 | User::$with → `roles` (目标) | BelongsToMany (Spatie) | `SELECT * FROM roles WHERE id IN (...)` |
| 11 | User::$with → `permissions` (pivot) | BelongsToMany (Spatie) | `SELECT * FROM model_has_permissions WHERE model_type = 'App\Models\User' AND model_id IN (...)` |
| 12 | User::$with → `permissions` (目标) | BelongsToMany (Spatie) | `SELECT * FROM permissions WHERE id IN (...)` |

#### 4.7.3 关于重复 SQL 的关键澄清

`album.artist` 存在**双重加载机制**：
...
| 层级 | SQL 数量 | 说明 |
|------|:-------:|------|
| 第一层（Song::$with） | 7 条 | albums、artists×2、podcasts、genre_song、genres、users |
...
| 「Eager Loading 约 6 条 SQL」 | **实际 12 条 SQL，低估 100%** | Laravel 递归加载关联模型自身的 `$with`：Album 隐式 `artist`（重复 1 条）+ User 的 `roles` + `permissions`（Spatie Permission，各 2 条共 4 条），额外增加 5 条 |
...
1. **在 owner 关联上禁用 User 的递归 `$with`**（成本：零成本，一行代码）：
   ```php
   $query->with(['owner' => fn ($q) => $q->without('roles', 'permissions')]);
   ```
...
3. **清理 `album.artist` 的冗余声明（去重查询）**（成本：极低）：
...
4. **用 `songs.artist_name` 冗余列替代 `artist->name`**（成本：极小，改一行 SongResource）：
   ```php
