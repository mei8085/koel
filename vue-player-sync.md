...
| Store | 职责 | 核心文件 |
|-------|------|----------|
| `playableStore` | 管理所有可播放项的元数据与播放状态 | [playableStore.ts](file:///d:/fz/0508-2/solo-dogfeeding/code/115-koel/resources/assets/js/stores/playableStore.ts) |
| `queueStore` | 管理播放队列（列表、当前项、上下首） | [queueStore.ts](file:///d:/fz/0508-2/solo-dogfeeding/code/115-koel/resources/assets/js/stores/queueStore.ts) |
| `commonStore` | 存储服务端下发的初始播放状态 | [commonStore.ts](file:///d:/fz/0508-2/solo-dogfeeding/code/115-koel/resources/assets/js/stores/commonStore.ts) |
...
状态迁移由 [QueuePlaybackService](file:///d:/fz/0508-2/solo-dogfeeding/code/115-koel/resources/assets/js/services/QueuePlaybackService.ts) 控制：
...
- [BasePlaybackService](file:///d:/fz/0508-2/solo-dogfeeding/code/115-koel/resources/assets/js/services/BasePlaybackService.ts) — 抽象基类，定义播放/暂停/停止等接口
- [QueuePlaybackService](file:///d:/fz/0508-2/solo-dogfeeding/code/115-koel/resources/assets/js/services/QueuePlaybackService.ts) — 队列播放实现
- [playbackManager](file:///d:/fz/0508-2/solo-dogfeeding/code/115-koel/resources/assets/js/services/playbackManager.ts) — 播放管理器，切换播放源
...
|---------|----------|----------|----------|
| 播放开始 | 每次切歌 | `PUT /api/queue/playback-status` | [QueuePlaybackService.ts#L209](file:///d:/fz/0508-2/solo-dogfeeding/code/115-koel/resources/assets/js/services/QueuePlaybackService.ts#L209-L212) |
| 播放进度 | 每 5 秒 | `PUT /api/queue/playback-status` | [QueuePlaybackService.ts#L421](file:///d:/fz/0508-2/solo-dogfeeding/code/115-koel/resources/assets/js/services/QueuePlaybackService.ts#L421-L425) |
| 队列变更 | 队列变化时 | `PUT /api/queue/state` | [queueStore.ts#L164](file:///d:/fz/0508-2/solo-dogfeeding/code/115-koel/resources/assets/js/stores/queueStore.ts#L164-L169) |
| 播放计数 | 播放超过 25% 时 | `POST /api/interaction/play` | [playableStore.ts#L155](file:///d:/fz/0508-2/solo-dogfeeding/code/115-koel/resources/assets/js/stores/playableStore.ts#L155-L160) |

### 1.5 初始化流程

应用启动时，通过 [FetchInitialDataController](file:///d:/fz/0508-2/solo-dogfeeding/code/115-koel/app/Http/Controllers/API/FetchInitialDataController.php) 获取初始数据，其中包含 `queue_state`：
...
音频播放路由定义在 [web.base.php](file:///d:/fz/0508-2/solo-dogfeeding/code/115-koel/routes/web.base.php) 中，使用独立的 `audio.auth` 中间件：
...
[PlayController](file:///d:/fz/0508-2/solo-dogfeeding/code/115-koel/app/Http/Controllers/PlayController.php) 是流式播放的入口：
...
[Streamer](file:///d:/fz/0508-2/solo-dogfeeding/code/115-koel/app/Services/Streamer/Streamer.php) 使用适配器模式，根据歌曲存储类型和转码需求选择不同的流式实现：
...
后端通过 [QueueState](file:///d:/fz/0508-2/solo-dogfeeding/code/115-koel/app/Models/QueueState.php) 模型持久化播放状态，存储在数据库中：
...
[QueueService](file:///d:/fz/0508-2/solo-dogfeeding/code/115-koel/app/Services/QueueService.php) 提供读写接口：
...
对应 API 控制器：
- [QueueStateController](file:///d:/fz/0508-2/solo-dogfeeding/code/115-koel/app/Http/Controllers/API/QueueStateController.php) — 队列状态读写
- [UpdatePlaybackStatusController](file:///d:/fz/0508-2/solo-dogfeeding/code/115-koel/app/Http/Controllers/API/UpdatePlaybackStatusController.php) — 播放进度更新
...
[CompositeToken](file:///d:/fz/0508-2/solo-dogfeeding/code/115-koel/app/Values/CompositeToken.php) 包含两个 Token：
...
登录时由 [TokenManager](file:///d:/fz/0508-2/solo-dogfeeding/code/115-koel/app/Services/Auth/TokenManager.php) 创建双 Token：
...
前端通过 [authService](file:///d:/fz/0508-2/solo-dogfeeding/code/115-koel/resources/assets/js/services/authService.ts) 接收并存储两个 Token：
...
**API 请求**（[http.ts](file:///d:/fz/0508-2/solo-dogfeeding/code/115-koel/resources/assets/js/services/http.ts)）：
...
**音频播放**（[playableStore.ts](file:///d:/fz/0508-2/solo-dogfeeding/code/115-koel/resources/assets/js/stores/playableStore.ts)）：
...
[AudioAuthenticate](file:///d:/fz/0508-2/solo-dogfeeding/code/115-koel/app/Http/Middleware/AudioAuthenticate.php) 中间件验证 Audio Token：
...
这里依赖 Laravel Sanctum 的 Token 解析能力——Sanctum 会自动从 URL 参数 `t` 或请求头中查找 Token 并解析出用户。

### 3.6 Token 注销

登出时同时删除两个 Token（[TokenManager::deleteCompositionToken](file:///d:/fz/0508-2/solo-dogfeeding/code/115-koel/app/Services/Auth/TokenManager.php)）：
...
浏览器发起 GET /play/{song}?t=<audio-token>
    │
    ▼
AudioAuthenticate 中间件
    │
    ├─ 从 ?t= 参数解析 Sanctum Token
...
timeupdate 事件（每秒触发，1000ms 节流）
    ...
    │   （携带 API Token 在 Authorization 头）
    │
...
| 场景 | 认证方式 | Token 类型 | 权限范围 |
|------|----------|-----------|----------|
| API 数据请求 | `Authorization: Bearer` Header | API Token | 全部权限 |
| 音频流播放 | URL 查询参数 `?t=` | Audio Token | 仅 audio 权限 |
| 初始数据加载 | API 请求 | API Token | 全部权限 |
| 播放进度同步 | API 请求 | API Token | 全部权限 |
